...
const ContainerDescriptionSneakers = ()=>{
    ...
    let {countSneakers,setCountSneakers,numberCount,setNumberCount} = useContext(contextSneakers);
    
    useEffect(()=>{
        if(numberCount <= 0 ){
            setNumberCount((value => value=0))
        }
    },[numberCount])
        ...
        <h2 className="title-company text-orange-400 font-[700] uppercase tracking-widest text-xs">Sneaker Company</h2>
            ...
            <div className="button-action-sneakers flex flex-col h-auto mt-5 xs:flex-row  xs:h-[50px] gap-3">
                <div className="count-sneakers w-full h-[40px] xs:h-auto xs:w-[175px] flex bg-ecommerce-light-grayish-blue rounded-md overflow-hidden">
                    <button className="min-sneaker group px-3  grid place-items-center" onClick={()=>{setNumberCount((count)=> count-=1)}}>
                        <img src="./images/icon-minus.svg" className="group-active:scale-90 transition-all duration-200" alt="" />
                    </button>
                    <span className="count-value flex-1 grid place-items-center">{numberCount}</span>
                    <button className="plus-sneaker group px-3  grid place-items-center transition-all duration-200" onClick={()=>{setNumberCount((count)=> count+=1)}}>
                        <img src="./images/icon-plus.svg" className="group-active:scale-90" alt="" />
                    </button>
                </div>
                <button onClick={()=>{
                    setCountSneakers(countSneakers+numberCount)
                    setNumberCount(0)
                }} className="add-to-cart transition-all duration-200 active:scale-90 h-[40px] flex items-center justify-center bg-ecommerce-orange rounded-md gap-2 font-[700] text-white text-sm xs:flex-1 xs:h- shadow-[0_1em_1em_0_rgba(221,221,221,0.8)]">
